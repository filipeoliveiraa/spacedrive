...
pub mod location {
	...
	impl SetParam {
		...
		fn with_param(&self) -> Option<crate::prisma::location::WithParam> {
			match self {
				_ => None
			}
		}
		...
		pub async fn exec(
			...
			let res = client
				...
				.create(
					...
					set_params.name.clone(),
					set_params._params.into_iter().map(Into::into).collect::<Vec<_>>(),
				)
				.exec()
				.await?;
							...
							match data {
								...
								::prisma_crdt::SharedOperationData::Update { field, value } => {
									...
									.update(
										...
										vec![serde_json::from_value::<__prisma_crdt::file::CRDTSetParam>(
								...
								::prisma_crdt::SharedOperationData::Update { field, value } => {
									self.client
										...
										.update(
											...
											vec![
												serde_json::from_value::<__prisma_crdt::tag::CRDTSetParam>(
													vec![(field, value)].into_iter().collect(),
												)
												.unwrap()
												.into(),
									...
									self.client
										...
										.update(
											...
											vec![serde_json::from_value::<
												__prisma_crdt::tag_on_file::CRDTSetParam,
											>(vec![(field, value)].into_iter().collect())
											.unwrap()
											.into()],
										)
